...
export const isOptionsKey = (key: string, options: Options): key is keyof Options => {
...
export const partition = <ArrayTypes, Validator extends ValidatorFn>(
  array: ArrayTypes[],
  isValid: Validator
): Partition<ArrayTypes, Validator> => {
  return array.reduce<Partition<ArrayTypes, Validator>>(
    (partitions, entry) => {
