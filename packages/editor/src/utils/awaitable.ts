...
export type AwaitableCallbacks = {
  fulfilled: AwaitableCallback[];
  rejected: AwaitableCallback[];
  settled: AwaitableCallback[];
};
...
export type AwaitableHandler<ResolvedValue = unknown, RejectedValue = unknown> = (
  resolve: AwaitableSettler<ResolvedValue>,
  reject: AwaitableSettler<RejectedValue>
) => any;
export type AwaitableState = {
  callbacks: AwaitableCallbacks;
  error?: any;
...
export const awaitable = <
  InitialValue extends object,
  ResolvedValue = InitialValue,
  RejectedValue = unknown
>(
  initialValue: InitialValue,
  handler: AwaitableHandler<ResolvedValue, RejectedValue>
): InitialValue & Promise<ResolvedValue> => {
  ...
  const callback = (
    settler: AwaitableThenSettler,
    { resolve, reject }: { reject: AwaitableSettler; resolve?: AwaitableSettler }
  ) => {
    return () => {
      try {
  ...
  const then = <OnFulfilled extends AwaitableThenSettler, OnRejected extends AwaitableThenSettler>(
