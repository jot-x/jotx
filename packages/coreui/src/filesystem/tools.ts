...
export const rsync = async (
  ...
  if (srcType === null) {
    ...
    const destType = 'async' in fs2 ? await fs2.getType(dest) : await toPromise(() => fs2.getType(dest))
    if (destType === 'file') {
    ...
    const fileData = 'async' in fs1 ? await fs1.readFile(src) : await toPromise(() => fs1.readFile(src))
